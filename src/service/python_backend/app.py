...
def get_answer_for_question(question, data):
    # Tentukan jenis pertanyaan dengan regex atau kata kunci
    
    # Pertanyaan tentang jumlah siswa
    if re.search(r'berapa (jumlah|banyak|total) siswa', question):
        return f"Terdapat {len(data)} siswa dalam dataset."
    
    # Pertanyaan tentang rata-rata skor
    if re.search(r'(rata-rata|rata rata|average) skor', question):
    ...
    if re.search(r'(distribusi|sebaran) umur', question):
    ...
    if re.search(r'(distribusi|sebaran) grade', question):
    ...
    if re.search(r'(korelasi|hubungan).*(skor|nilai).*absensi', question) or \
       re.search(r'(korelasi|hubungan).*absensi.*(skor|nilai)', question):
    ...
    if re.search(r'siswa (dengan|yang) (skor|nilai) tertinggi', question):
    ...
    if re.search(r'siswa (dengan|yang) absensi (terendah|paling sedikit)', question):
    ...
    if re.search(r'(rekomendasi|saran|strategi) gamifikasi', question):
    ...
    if re.search(r'(ceritakan|jelaskan|informasi|gambaran).*(tentang|mengenai) dataset', question):
    ...
    return "Maaf, saya tidak dapat memahami pertanyaan Anda. Anda dapat bertanya tentang jumlah siswa, rata-rata skor, distribusi umur, korelasi skor dan absensi, atau rekomendasi strategi gamifikasi."
